...
def show_transcript_view_page():
    ...
    if (
        "transcription_results" not in st.session_state
        or not st.session_state.transcription_results
    ):
        st.warning(
            """
...
def show_full_transcript(transcription_results: List[Dict[str, Any]]):
    ...
    full_text = "\n".join(
        [item.get("text", "") for item in transcription_results if "text" in item]
    )
...
def show_visual_data(transcription_results: List[Dict[str, Any]]):
    ...
    df = pd.DataFrame(
        {
            ...
            "Text Length": [
                len(item.get("text", "")) for item in transcription_results
            ],
        }
    )
    ...
    if any("description" in item for item in transcription_results):
        df["Has Description"] = [
            1 if "description" in item else 0 for item in transcription_results
        ]

    # Display simple bar chart
    st.subheader("Text Length by Segment")
    st.bar_chart(df.set_index("Segment")["Text Length"], use_container_width=True)
